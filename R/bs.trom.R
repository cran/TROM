bs.trom <-
function(sp1_gene_expr=NULL, sp2_gene_expr=NULL, sp1_sp2_orthologs, z_thre=1.5, FPKM_thre=1,
  ...
  if (provide==FALSE){
    ...
    sp1_associated_idx <- sapply(2:ncol(sp1_ortholog_z), FUN=function(i) which(sp1_ortholog_z[,i]>z_thre & sp1_ortholog_data[,i]>FPKM_thre), simplify=FALSE)
    sp2_associated_idx <- sapply(2:ncol(sp2_ortholog_z), FUN=function(i) which(sp2_ortholog_z[,i]>z_thre & sp2_ortholog_data[,i]>FPKM_thre), simplify=FALSE)
