ws.trom.orthologs <-
function(sp1_sp2_orthologs, sp_gene_expr = NULL, z_thre = 1.5, FPKM_thre =1, i, 
                            provide = FALSE, gene_lists = NULL, save_overlap_genes = FALSE){
  ####################### no provided gene lists #####################
  if (provide == FALSE){
    ...
    sp_associated_idx <- sapply(2:ncol(sp_ortholog_z), FUN=function(i) which(sp_ortholog_z[,i]>z_thre & sp_ortholog_data[,i]>FPKM_thre), simplify=FALSE)
